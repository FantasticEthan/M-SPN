...
for t=1:opts.numCycles
    ...
    for seq_id=1:K
        ...
        res = mdnet_simplenn(net, im, flag, one, res, ...
        ...
        index = res(19).idx;
        info.train = updateError(info.train, t, index, labels, res, batch_time) ;
        
        fprintf(' %.2f s (%.1f images/s),', batch_time, speed) ;
        fprintf(' objective %.3f, error %.3f\n', ...
            info.train.objective(index,t)/(t*opts.batchSize),...
            info.train.error(index,t)/(t*opts.batchSize)) ;
%         info.train.objective(index,t)/(t*opts.batchSize),...
...
% info.error(1,t) = sum(sum(sum(error(:,:,1,:))))/n ;
