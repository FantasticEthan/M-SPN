function [ result ,fps] = mspn_run(num_frames,images, region, net, display)
% mspn_RUN
% Main interface for mspn tracker
%
% INPUT:
%   images  - 1xN cell of the paths to image sequences
%   region  - 1x4 vector of the initial bounding box [left,top,width,height]
%   net     - The path to a trained mspn
...
[net_conv, net_fc, opts] = mspn_init(img, net);
...
if(opts.bbreg)
    ...
    feat_conv = mspn_features_convX(net_conv, img, pos_examples, opts);
...
feat_conv = mspn_features_convX(net_conv, img, examples, opts);
...
% net_fc = mspn_finetune_hnm(net_fc,pos_data,neg_data,opts,...
%     'maxiter',opts.maxiter_init,'learningRate',opts.learningRate_init);
% 
% fprintf('learning CNN time....')
...
for To = 2:nFrames;
    ...
    feat_conv = mspn_features_convX(net_conv, img, samples, opts);
    

    % evaluate the candidates

    feat_fc = mspn_features_fcX(net_fc, feat_conv, opts);
